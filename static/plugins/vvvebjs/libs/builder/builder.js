...
Vvveb.Components = {
    ...
    render: function (type) {
        ...
        if (!(Vvveb.preservePropertySections && section.length)) {
            ...
            componentsPanelSections[defaultSection].html('').append(tmpl("vvveb-input-sectioninput", {
                key: "default",
        ...
        if (component.name === "Paragraph" || component.name === "Element" || component.name === "Jumbotron" || component.name === "Card" || component.name === "List Group" || component.name === "List Item") {
            componentsPanelSections[defaultSection].find('[data-header="default"]').hide();
        } else {
            componentsPanelSections[defaultSection].find('[data-header="default"]').show();
        ...
        var fn = function (component, property) {
            ...
            return property.input.on('propertyChange', function (event, value, input) {
                ...
                console.log("dsdf");
                $("#iframe1").contents().find('#clock').countdown('2021/1/10').on('update.countdown', function (event) {
                    console.log("here");
                ...
                jQuery("#iframe1").contents().find("video")[0].load();
                if (jQuery("#iframe1").contents().find("video")[1]) {
                    jQuery("#iframe1").contents().find("video")[1].load();
                    jQuery("#iframe1").contents().find("video")[2].load();
                    jQuery("#iframe1").contents().find("video")[3].load();
                    jQuery("#iframe1").contents().find("video")[4].load();
                    jQuery("#iframe1").contents().find("video")[5].load();
                    jQuery("#iframe1").contents().find("video")[6].load();
                    jQuery("#iframe1").contents().find("video")[7].load();
                    jQuery("#iframe1").contents().find("video")[8].load();
                    jQuery("#iframe1").contents().find("video")[9].load();
                    jQuery("#iframe1").contents().find("video")[10].load();
                }
...
Vvveb.Builder = {
                ...
                for (i in components) {
                    ...
                    if (component) {
                        ...
                        if (component.image) {
                            ...
                            item.css({
                                backgroundImage: "url(" + '/static/plugins/vvvebjs/libs/builder/' + component.image + ")",
                ...
                for (i in blocks) {
                    ...
                    if (block) {
                        ...
                        if (block.image) {
                            ...
                            item.css({
                                backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? '/static/plugins/vvvebjs/libs/builder/' : '') + block.image + ")",
                                backgroundRepeat: "no-repeat"
                            })
                        }
    ...
    selectNode: function (node) {
        ...
        if (target) {
            var old = self.selectedEl;
            self.selectedEl = target;

            try {
                // var start = jQuery("#wysiwyg-editor").attr("style");
                // console.log(target);
                // if(start !== "display: block;") {
                // 	console.log(start);
                // 	if (old === null || old[0] === target[0]) {
                ...
                // 			if (temp.includes("--builder-right-panel-width")) {
                // 				Vvveb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
                // 				$("#vvveb-builder").toggleClass("no-right-panel");
                // 				console.log("here3");
                //
                ...
                // 	}
                // }
                if (old[0] !== target[0] && !$("body").attr("style").includes("--builder-right-panel-width") && jQuery("#right-panel").attr("permanent") !== "on") {
                    Vvveb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
                    $("#vvveb-builder").toggleClass("no-right-panel");
                }
                ...
                if (target[0].tagName === "INPUT" || target[0].tagName === "SELECT" || (target[0].tagName === "DIV" && target.children("input").length > 0) || (target[0].tagName === "DIV" && target.children("select").length > 0)) {
                    jQuery("#drag-btn").hide();
                    jQuery("#parent-btn").hide();
                    jQuery("#up-btn").hide();
                    jQuery("#down-btn").hide();
                    jQuery("#clone-btn").hide();
                    jQuery("#delete-btn").hide();
                } else {
                    jQuery("#drag-btn").show();
                    jQuery("#parent-btn").show();
                    jQuery("#up-btn").show();
                    jQuery("#down-btn").show();
                    jQuery("#clone-btn").show();
    ...
    _initBox: function () {
        ...
        $("#down-btn").on("click", function (event) {
            ...
            console.log(self.selectedEl.get(0))
        ...
        $("#properties-btn").on("click", function (event) {
            Vvveb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
            $("#vvveb-builder").toggleClass("no-right-panel");
            if (jQuery("#right-panel").attr("permanent") === "on") {
                jQuery("#right-panel").attr("permanent", "off");
            }
        ...
        $("#add-section-btn").on("click", function (event) {
            ...
            var outerHeight = $(window).height() + self.frameDoc.scrollTop();
    ...
    saveAjax: function (fileName, startTemplateUrl, callback) {
        ...
        iframe.find("#immediate_assistance_fields").hide();
        iframe.find("#oc-testi").addClass("owl-carousel");
        iframe.find(".oc-testi").addClass("owl-carousel");
        iframe.find("#testimonial_class").addClass("flexslider");
        iframe.find(".testimonial_class").addClass("flexslider");
        if (site_name === "XBrownStone") {
            iframe.find("#slider").removeClass("slider_class");
        }
        if (site_name === "Dark") {
            iframe.find("#banner_title").parent().addClass("slider-caption");
            iframe.find("#banner_title").parent().removeClass("text-center");
            iframe.find(".running-car").css("left", "0px")
        }
        if (site_name === "Business") {
            iframe.find("#banner_title").parent().addClass("slider-caption");
            iframe.find("#banner_title").parent().removeClass("text-center");
            iframe.find(".slider-caption-center").removeClass("custom-margin");
        }
        data['head'] = iframe.find("head").html();
        data['body'] = iframe.find("body").html();

        var csrftoken = getCookie('csrftoken');
        $.ajax({
            headers: {"X-CSRFToken": csrftoken},
            type: "POST",
            url: '/xsite/save_template/' + site_id + '/',//set your server side save script url
            data: data,
            cache: false,
            success: function (data) {
                console.log("in success");
                iframe.find("#immediate_assistance_fields").show();
                iframe.find("#oc-testi").removeClass("owl-carousel");
                iframe.find(".oc-testi").removeClass("owl-carousel");
                iframe.find("#testimonial_class").removeClass("flexslider");
                iframe.find(".testimonial_class").removeClass("flexslider");
                if (site_name === "Dark") {
                    iframe.find("#banner_title").parent().removeClass("slider-caption");
                    iframe.find("#banner_title").parent().addClass("text-center");
                    iframe.find(".running-car").css("left", "659px")
                }
                if (site_name === "Business") {
                    iframe.find("#banner_title").parent().removeClass("slider-caption");
                    iframe.find("#banner_title").parent().addClass("text-center");
                    if ($("body").attr("style").includes("--builder-right-panel-width") || $("body").attr("style").includes("--builder-left-panel-width")) {
                        iframe.find(".slider-caption-center").addClass("custom-margin");
                    } else {
                        iframe.find(".slider-caption-center").removeClass("custom-margin");
                    }
                }
                if (site_name === "XBrownStone") {
                    iframe.find("#slider").addClass("slider_class");
                }
...
Vvveb.Gui = {
    ...
    saveAjax: function () {
        ...
        return Vvveb.Builder.saveAjax(url, null, function (data) {
            $('#message-modal').modal().find(".modal-body").html("Template Saved Successfully!");
        });
    },
    ...
    preview: function () {
        ...
        if (jQuery("#right-panel").attr("style") === "display: block;" || jQuery("#right-panel").attr("panel") === "yes") {
            if (jQuery("#right-panel").attr("panel") === "yes") {
                jQuery("#right-panel").attr("panel", "no");
            } else {
                jQuery("#right-panel").attr("panel", "yes");
            }
            Vvveb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
            $("#vvveb-builder").toggleClass("no-right-panel");
        }
...
//Pages, file/components tree 
    ...
    setDesignerMode: function () {
        ...
        var designerMode = this.attributes["mydata"].value !== "true";
        $("#designer-mode-btn").attr('mydata', designerMode);
...
//layout
    togglePanel: function (panel, cssVar) {
        ...
        if (panel.is(":visible")) {
            ...
            if (site_name === "Violet") {
                var temp = body.attr("style");
                if (typeof temp !== "undefined") {
                    temp = temp + cssVar + ':0px;';
                    body.removeAttr("style");
                    body.attr("style", temp);
                } else {
                    body.attr("style", cssVar + ':0px;');
                }
            }
        ...
        if ($("body").attr("style").includes("--builder-right-panel-width") || $("body").attr("style").includes("--builder-left-panel-width")) {
            $("#iframe1").contents().find(".slider-caption-center").addClass("custom-margin");
        } else {
            $("#iframe1").contents().find(".slider-caption-center").removeClass("custom-margin");
        }
    },
    ...
    toggleRightColumn: function () {
        Vvveb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
        $("#vvveb-builder").toggleClass("no-right-panel");
        if ((typeof jQuery('#right-panel').attr("permanent") === "undefined" || jQuery('#right-panel').attr("permanent") === "off") && jQuery('#right-panel').attr("style") === "display: block;") {
            jQuery('#right-panel').attr("permanent", "on");
        } else {
            jQuery('#right-panel').attr("permanent", "off");
        }
    },
    ...
    load_site: function () {
        console.log("in load site");
        var site = $(this).attr('mydata');

        if (load_type === "default") {
            window.location.href = '/xsite/get_edit_site/edited/' + site + '/';
        } else {
            window.open('/xsite/get_edit_site/edited/' + site + '/', '_blank')
        }
        ...
        function drawComponentsTree(tree) {
            ...
            for (i in tree) {
                ...
                if (tree[i].children.length > 0) {
					...
					<label for="id' + j + '" style="background-image:url(/static/plugins/vvvebjs/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
    ...
    loadPage: function (name, allowedComponents = false, disableCache = true) {
        ...
        Vvveb.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1 ? '&' : '?') + Math.random() : ''),
            function () {
                ...
                // 		var iframe = $('#iframe1').contents();
                //
                // 		iframe.find("head").append(data['template_content'].html_head_content);
                // 		iframe.find("body").append(data['template_content'].html_body_content);
                // 		if (!data['template_content'].is_edited) {
                // 			iframe.find("#logo").attr("href", data['template_content'].logo);
                // 			iframe.find("#phone").html(data['phone']);
                // 			iframe.find("#banner_title").html(data['template_content'].banner_title);
                // 			iframe.find("#about_us_title").html(data['template_content'].about_us_title);
                // 			iframe.find("#about_us_desc").html(data['template_content'].about_us_desc);
                // 			iframe.find("#video_link").attr("href", data['template_content'].video_link);
                // 			iframe.find("#video_title").html(data['template_content'].video_title);
                // 			iframe.find("#video_desc").html(data['template_content'].video_desc);
                // 			iframe.find("#testi_1_company_name").html(data['template_content'].testi_1_company_name);
                // 			iframe.find("#testi_1_person_name").html(data['template_content'].testi_1_person_name);
                // 			iframe.find("#testi_1_text").html(data['template_content'].testi_1_text);
                // 			iframe.find("#testi_2_company_name").html(data['template_content'].testi_2_company_name);
                // 			iframe.find("#testi_2_person_name").html(data['template_content'].testi_2_person_name);
                // 			iframe.find("#testi_2_text").html(data['template_content'].testi_2_text);
                // 			iframe.find("#testi_3_company_name").html(data['template_content'].testi_3_company_name);
                // 			iframe.find("#testi_3_person_name").html(data['template_content'].testi_3_person_name);
                // 			iframe.find("#testi_3_text").html(data['template_content'].testi_3_text);
                // 			iframe.find("#template-contactform-submit").html(data['template_content'].banner_button_text);
                // 		}
