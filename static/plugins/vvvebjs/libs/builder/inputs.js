		...
		if (this.files && this.files[0]) {
			...
			if(type[0] === "image"){
				...
				$.ajax({
					...
					type: "POST",
					url: '/xsite/save_image',//set your server side upload script url
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
						console.log("File uploaded at: ", data['uploaded_file_url']);

						//if image is succesfully uploaded set image url
						event.data.element.trigger('propertyChange', [data['uploaded_file_url'], this]);
						//update src input
						$('input[type="text"]', event.data.element).val(data['uploaded_file_url']);
					},
					error: function (data) {
